...
static void runPipelineThread(){
    ...
    push("已生成训练脚本: " + g_out + "/train.py");
    push(ok ? "=== 处理完成，可以开始训练 ===" : "=== 处理失败 ===");
    g_busy = false;
    if(g_btnRun) gtk_widget_set_sensitive(g_btnRun, TRUE);
    if(g_status) gtk_label_set_text(GTK_LABEL(g_status), ok ? "处理完成" : "处理失败");
}

static void onRun(GtkWidget* btn, gpointer){
    if(g_busy) return;
    if(g_sing.empty() || g_speech.empty()){ push("请先选择两个文件夹"); return; }
    if(g_out.empty()){ push("请先选择输出文件夹"); return; }
    ...
    if (cr == CheckResult::Refuse) {
        gtk_label_set_text(GTK_LABEL(g_status), "已拒绝：输入不符合要求");
        return;
    }
    ...
    gtk_label_set_text(GTK_LABEL(g_status), "处理中…");
...
static void onTrain(GtkWidget*, gpointer){
    ...
    if(!p){ push("无法启动 python3（训练需要 torch，本机大概率没有）"); return; }
    char buf[512];
...
static void onVerify(GtkWidget*, gpointer){
    GtkWidget* d = gtk_file_chooser_dialog_new("选择 .onnx 模型", nullptr,
    ...
    if(gtk_dialog_run(GTK_DIALOG(d)) == GTK_RESPONSE_ACCEPT){
        ...
        push(std::string("验证 ") + f + (e.ready() ? "  加载成功" : "  加载失败: " + e.lastError()));
        if(e.ready()){
            ...
            if(e.predictF0(pcm,44100.0,f0))
                push("  按 F0 模型解释成功，输出 " + std::to_string(f0.size()) + " 帧");
            else
                push("  按 F0 模型解释失败（可能是润色模型）: " + e.lastError());
        }
        g_free(f);
    }
    gtk_widget_destroy(d);
...
int main(int argc, char** argv){
    ...
    gtk_window_set_title(GTK_WINDOW(win), "VoxEngine 全自动训练流水线");
    ...
    const char* labels[3] = { "歌唱模板文件夹", "说话样本文件夹", "输出文件夹" };
    ...
    for(int i=0;i<3;i++){
        ...
        GtkWidget* fc = gtk_file_chooser_button_new("选择文件夹", GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER);
    ...
    gtk_box_pack_start(GTK_BOX(req), gtk_label_new("时长门槛：歌唱不少于"), FALSE, FALSE, 0);
    ...
    gtk_box_pack_start(GTK_BOX(req), gtk_label_new("分钟，说话不少于"), FALSE, FALSE, 0);
    ...
    g_status = gtk_label_new("就绪：选两个文件夹，点①");
